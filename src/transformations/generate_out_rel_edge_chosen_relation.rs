...
impl<'a, 'b> GenerateOutgoingRelationalEdgeChosenBodyLiteral<'a, 'b> {
    /* fn fetch_adg(self) -> &'a mut AnnotatedDependencyGraph {
        self.adg
    } */
    pub fn name(&self) -> String {
        String::from(format!(
            "Generating an Outgoing Relational Edge"
        ))
    }
    ...
    pub fn new(
        ...
        let chosen_rule_name: String = incoming_edges_by_rule_name
            .keys()
            .choose(rng)
            .expect(format!("No rules for this relation {}", chosen_head_rel.name()).as_str())
            .clone();
        ...
        Some(Self {
            ...
            chosen_body_rel: chosen_body_rel.clone(),
            sign: match is_pos {
                true => Sign::Positive,
                false => Sign::Negative,
            },
...
impl<'a, 'b> ProgramTransformation for GenerateOutgoingRelationalEdgeChosenBodyLiteral<'a, 'b> {
    fn apply(self, program: &ProgramHandle) -> Result<ProgramHandle, ValidationReport> {
        info!(
            "  Adding outgoing relational edge from {} for {}",
        ...
        let new_rel_arity: usize = arities
            .get(&self.chosen_body_rel)
            .expect("All relations should have an arity by now!")
            .clone();
        ...
        let new_lit: Literal = match self.sign.is_positive() {
            true => Literal::Positive(Atom::new(self.chosen_body_rel.clone(), generated_lit_vars)),
            false => Literal::Negative(Atom::new(self.chosen_body_rel.clone(), generated_lit_vars)),
        };
