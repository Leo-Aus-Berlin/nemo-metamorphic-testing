            ...
            match self.rng.random_bool(0.5) {
                ...
                true => match self.adg.get_ground_terms().choose(self.rng) {
                    None => {
                        ...
                        let new_gt = self.adg.get_and_register_new_iri_constant();
                ...
                false => {
                    match self.rng.random_bool(0.5) {
                        ...
                        // new integer or string
                        false => match self.rng.random_bool(0.5) {
                            // integer
                            true => {
                                let new_gt =
                                    self.adg.get_and_register_new_integer_constant(self.rng);
                                terms.push(Term::Primitive(Primitive::Ground(new_gt)));
                            }
                            // string
                            false => {
